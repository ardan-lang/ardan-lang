...
enum ValueTag : uint8_t {
    TAG_NUMBER        = 0b001, // Standard number (int or double)
    TAG_STRING        = 0b000,
    TAG_BOOLEAN       = 0b010,
    TAG_UNDEFINED     = 0b011,
    TAG_OBJECT        = 0b100,
    TAG_NULL          = 0b101,
    TAG_ARRAY         = 0b110,
    TAG_FUNCTION      = 0b111, // User-defined or native function
    TAG_SYMBOL        = 0b1000,
    TAG_BIGINT        = 0b1001,
    TAG_DATE          = 0b1010,
    TAG_REGEXP        = 0b1011,
...
#pragma pack(push,1)
struct ValueTagStruct {
    uint8_t tag;      // offset 0
    uint8_t pad[7];   // offset 1..7
    uint64_t val;     // offset 8..15
};

#pragma pack(pop)
static_assert(sizeof(ValueTagStruct) == 16, "ValueTagStruct must be 16 bytes");

inline ValueTagStruct make_int(int64_t i) {
    ValueTagStruct v{};
    v.tag = TAG_NUMBER;
    v.val = (uint64_t)i;
    return v;
...
//inline ValueTagStruct make_double(double d) {
//    ValueTagStruct v{};
//    v.tag = TAG_DOUBLE;
//    uint64_t bits;
...
inline ValueTagStruct make_ptr(ValueTag tag, void* p) {
    ValueTagStruct v{};
    v.tag = (uint8_t)tag;
    v.val = (uint64_t)(uintptr_t)p;
    return v;
}

inline ValueTagStruct make_bool(bool b) {
    ValueTagStruct v{};
    v.tag = TAG_BOOLEAN;
    v.val = b ? 1 : 0;
    return v;
...
// 1 byte (tag) + 7 bytes padding + 8 bytes (union), so typically 16 bytes total on a 64-bit system due to alignment
//struct ValueTagStruct {
//    ValueTag tag; // type tag
//
//    union {
//        int64_t    intValue;     // For TAG_NUMBER (integer)
//        double     doubleValue;  // For TAG_NUMBER (floating point)
//        bool       boolValue;    // For TAG_BOOLEAN
//        void*      ptrValue;     // For TAG_STRING, TAG_OBJECT, etc.
//    };
//
//    ValueTagStruct() : tag(TAG_UNDEFINED), ptrValue(nullptr) {}
//
//    // Constructors for each type:
//    explicit ValueTagStruct(int64_t v)
//        : tag(TAG_NUMBER), intValue(v) {}
//
//    explicit ValueTagStruct(double v)
//        : tag(TAG_NUMBER), doubleValue(v) {}
//
//    explicit ValueTagStruct(bool v)
//        : tag(TAG_BOOLEAN), boolValue(v) {}
//
//    explicit ValueTagStruct(ValueTag t, void* p)
//        : tag(t), ptrValue(p) {}
//
//    // Factory methods for common cases
//    static ValueTagStruct makeString(const char* s) {
//        ValueTagStruct v;
//        v.tag = TAG_STRING;
//        v.ptrValue = (void*)s;
//        return v;
//    }
//    static ValueTagStruct makeNull() {
//        ValueTagStruct v;
//        v.tag = TAG_NULL;
//        v.ptrValue = nullptr;
//        return v;
//    }
//    static ValueTagStruct makeUndefined() {
//        ValueTagStruct v;
//        v.tag = TAG_UNDEFINED;
//        v.ptrValue = nullptr;
//        return v;
...
//    // Add more as needed...
//
//    // Conversion helpers (optionally, type check before use)
//    int64_t asInt() const { return intValue; }
//    double asDouble() const { return doubleValue; }
//    bool asBool() const { return boolValue; }
//    void* asPtr() const { return ptrValue; }
//};

struct ValueTagS {
    uint64_t raw;  // raw tagged value
};
static_assert(sizeof(ValueTagS) == 8);
...
public:
    ...
    uint8_t getTag(ValueTagS v) {
        return v.raw & 0b111;
    }

    uint64_t getPayload(ValueTagS v) {
        return v.raw >> 3;
    }
    ...
    ValueTagS makeStringValue(size_t addr) {
        ValueTagS v;
        v.raw = ((uint64_t)addr << 3) | TAG_STRING;  // pointer tagging
        return v;
    }
    
    ValueTagS addString(const std::string& s) {
        size_t addr = addStringData(s);
        return makeStringValue(addr);
    }

    ValueTagS makeIntValue(int64_t i) {
        ValueTagS v;
        v.raw = ((uint64_t)i << 3) | TAG_NUMBER;
        return v;
    }

    const char* getStringPtr(ValueTagS v) {
    ...
    size_t addValue(const ValueTagS& v) {
        size_t addr = dataSection.size();
        uint64_t raw = v.raw;
    ...
    size_t addNumericData_(const int value) {
        alignDataSection();
        size_t addr = (uint64_t)(dataSection.size());
        // dataSection.push_back((uint64_t)value);
        dataSection.insert(dataSection.end(), (uint8_t)value, (uint8_t)value + sizeof(uint64_t));
        dataSection.push_back('\0');
...
//    
    ...
    size_t addValueTagStruct(const ValueTagStruct& v) {
        size_t addr = dataSection.size();
        const uint8_t* bytes = reinterpret_cast<const uint8_t*>(&v);
        dataSection.insert(dataSection.end(), bytes, bytes + sizeof(ValueTagStruct));
        return addr;
    }
