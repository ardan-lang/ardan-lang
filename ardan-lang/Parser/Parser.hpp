...
private:
    ...
    unique_ptr<Expression> parseAssignment() {
        ...
        if (match(TokenType::YIELD)) {
            ...
            return make_unique<YieldExpression>(yieldToken, std::move(expr), delegate);
        }
...
//            }
        ...
        if (match(TokenType::LEFT_SQUARE_BRACKET)) {
            ...
            if (!check(TokenType::RIGHT_SQUARE_BRACKET)) {
                do {
                    if (match(TokenType::SPREAD)) {
                        elements.push_back(parseSpread());
                    } else {
        ...
        if (match(TokenType::LEFT_BRACKET)) { // { object literal }
            ...
            if (!check(TokenType::RIGHT_BRACKET)) {
                do {
                    ...
                    Token key;
                    if (match(TokenType::SPREAD)) {
                        props.push_back({ key, parseSpread() });
                    } else {
    ...
    unique_ptr<Expression> parseAsyncExpression() {
        ...
        } else if (auto function_expr = dynamic_cast<FunctionExpression*>(expr.get())) {
            function_expr->is_async = true;
        } else {
            throw runtime_error("Async can only be applied on a function.");
        }
